...
/**
 * Key code to storage-style name (V, F1, MOUSE.1) — same as MaLiLib KeyCodes mapping.
 */
public final class HotkeyKeyCodes
{
    private static final Map<Integer, String> MAP_KEY_TO_NAME = new HashMap<>();
    private static final Map<String, Integer> MAP_NAME_TO_KEY = new HashMap<>();
    ...
    {
        return MAP_NAME_TO_KEY.getOrDefault(name, KEY_NONE);
    }

    static
    {
        for (Field field : HotkeyKeyCodes.class.getDeclaredFields())
        {
            try
            {
                if (java.lang.reflect.Modifier.isStatic(field.getModifiers()) == false) continue;
